...
doorways (no segment crosses a wall). More figures and an HD RViz capture in
[`docs/report/figures/`](docs/report/figures/); the full report and slides are in
[`docs/soutenance/`](docs/soutenance/).*
...
| ----------------------- | :-----: | ------------------------------------------------------------- |
| *(none - default)*      |    -    | **Autonomous 2-phase mission** (L18): phase 1 frontier exploration → map; phase 2 inspects each discovered room at poses **derived from the map at runtime** (no victim coordinates) → all 4 victims. One click, fully autonomous |
| `IA712_INSPECT`         | `1`     | `0` = skip phase 2 (pure exploration only; ~2 victims - see decision journey) |
| `IA712_WAYPOINTS`       | `0`     | `1` = follow a generic coverage waypoint route instead (optional/manual fallback) |
| `IA712_HYBRID`          | `0`     | `1` = explore, then sweep a **generic** coverage route (no victim coordinates) |
...
9. **OOM root cause - the Gazebo leak:** runs > ~10 min OOM because **`mem_profile.sh` proved Gazebo leaks ~35 MB/s** (Ogre2/D3D12+WSL render loop, per *wall*-second). Isolating the leaker showed the **`gpu_lidar` sensor dominates** (removing it → flat memory); the real fix is **`gpu_lidar 62→10 Hz`**, which halves the leak and keeps RTF ~0.99 - runs now finish well before any OOM. A first nominal run (hybrid explore→patrol) then reached **90.6 % + 3 victims (ids 0,2,3) + annotated map, no OOM** (archived in `results/examples/l18_nominal/`).

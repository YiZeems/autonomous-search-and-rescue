...
\begin{frame}{Fondements théoriques (issus du cours)}
  ...
  {\small\begin{itemize}
    \item \textbf{SLAM + fermeture de boucle} - carte en ligne cohérente (CM6--7).
    \item \textbf{Exploration par frontières} - limite libre/inconnu ; \emph{gain d'information}
      $g(f)=\text{gain}-\lambda\,\text{coût}$ (CM8). Liste noire des frontières mortes (échec Nav2, ou 3$\times$ sans gain, écartée).
    \item \textbf{Planification de chemin} - Dijkstra/NavFn vs.\ A* ; NavFn conservé (tolérant en espace étroit) (CM9).
    \item \textbf{TF2} - projection détection caméra vers le repère global \texttt{map} (CM, exigence).
...
\begin{frame}{Voir l'algorithme : Gazebo (simulateur) vs RViz (frontières)}
  \begin{columns}[c]
  ...
  \end{columns}
  \smallskip
  {\footnotesize\color{nccblue}\centering Comme en TP (CM8) : chaque \emph{cluster} de frontières a sa couleur, le robot vise la meilleure utilité $g=\text{gain}-\lambda\,\text{coût}$, et l'on \emph{voit} le liseré grignoter l'inconnu jusqu'à $>90\,\%$. Captures : \texttt{gazebo\_capture.mp4} \& \texttt{rviz\_capture.mp4}.}
\end{frame}
...
\begin{frame}{Démonstration}
  ...
  \href{https://www.youtube.com/watch?v=BKXprWFloFs}{\includegraphics[width=0.60\linewidth]{youtube_demo.jpg}}\\[3mm]

...
\begin{frame}{The mission: autonomous search \& rescue}
  ...
  \textbf{Scenario~[1]:} a robot explores an unknown disaster zone, finds ``victims'' (AprilTags),
  ...
  \begin{itemize}
    ...
    \item Camera detection $\to$ \texttt{map} via \texttt{tf2}
  ...
  \end{columns}
  \vfill
  {\footnotesize\color{nccblue}\textbf{Result:} one continuous autonomous run - \textbf{97.17\,\% coverage, 4/4 victims}.}
\end{frame}
...
\begin{frame}{System architecture - four ROS\,2 packages}
  ...
  \begin{tikzpicture}[node distance=5mm and 9mm,scale=0.9,transform shape]
    ...
    \node[box,right=of slam] (metric) {coverage};
    ...
    \node[io,right=of bt] (rviz) {RViz / disk};
  ...
  \end{tikzpicture}
  \par\medskip
  {\footnotesize The \textbf{Behavior Tree orchestrates}: it gates the explorer and the inspector via
  \texttt{/mission/*} topics; the Python nodes do the work.}
\end{frame}
...
\begin{frame}{Two-phase mission orchestrated by the Behavior Tree}
  ...
  never gets within 2\,m of the wall tags $\Rightarrow$ it caps at $\sim$2 victims.}
\end{frame}
...
\begin{frame}{Theoretical foundations (from the course)}
  ...
  \begin{itemize}
    \item \textbf{SLAM + loop closure} - consistent online map (CM6--7)~[4].
    \item \textbf{Frontier exploration} - free/unknown boundary~[2]; \emph{information-gain}
    ...
    \item \textbf{TF2} - project camera detection $\to$ global \texttt{map} (CM, requirement~[8]).
    \item \textbf{Behavior Tree} - reactive, composable; memory \texttt{Sequence}, not an FSM~[6].
  \end{itemize}
  \end{column}
...
\begin{frame}{The decision journey - three runs}
  ...
  \node[draw,fill=nccgreen!8,text width=4.9cm,align=left,inner sep=3pt,below=2mm of b2](i2){\textbf{\color{nccgreen}Improved:} map-derived inspection; \textbf{BT orchestrates} $\Rightarrow$ \textbf{4/4 + 97\,\%}.};
  \node[draw,fill=nccred!8,text width=4.9cm,align=left,inner sep=3pt](b3) at (13.65,2.9){\textbf{\color{nccred}Blocked:} RTF~1.5 $\to$ 3/4 + GPU stress; figures looked wall-crossing.};
...
\begin{frame}{Results - live algorithms in RViz}
  ...
  \par\smallskip
  {\footnotesize Exploration $\to$ inspection $\to$ four victims; bottom-right: the real map-frame path
  (walls drawn over it, so it visibly threads doorways - no segment crosses a wall).}
\end{frame}
...
\begin{frame}{Results - coverage, map and conformity}
  ...
  \includegraphics[width=\linewidth]{coverage_curve}\\{\scriptsize Coverage \& path vs.\ time, 90\,\% target.}
  \end{column}
  \begin{column}{0.5\linewidth}\centering
  \includegraphics[width=0.86\linewidth]{mission_map}\\{\scriptsize Final map: 4 victims + autonomous inspection tour.}
  \end{column}
  \end{columns}
  \medskip
  {\footnotesize\color{nccblue}\centering \textbf{97.17\,\% coverage $\cdot$ 4/4 victims $\cdot$ annotated map $\cdot$ logged trajectory} - one click, fully autonomous.}
\end{frame}
...
\begin{frame}{Team and collaboration}
  ...
  \end{tikzpicture}\\
  {\scriptsize Three workstreams $\to$ \texttt{bl} integration.}
  \end{column}
  \end{columns}
\end{frame}

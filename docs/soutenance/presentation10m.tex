...
\begin{frame}{Theoretical foundations}
  ...
  {\small\begin{itemize}
    \item \textbf{SLAM + loop closure} - consistent online map.
    \item \textbf{Frontier exploration} - free/unknown boundary; \emph{information-gain}
      $g(f)=\text{gain}-\lambda\,\text{cost}$. Dead-frontier blacklist
      (Nav2 failure, or 3$\times$ with no gain, blacklisted).
    \item \textbf{Path planning} - Dijkstra/NavFn vs.\ A*; NavFn kept (tolerant in tight space).
    \item \textbf{TF2} - project camera detection to global \texttt{map} (project requirement).
...
\begin{frame}{Seeing the algorithm: Gazebo vs RViz}
  \begin{columns}[c]
  ...
  \end{columns}
  \smallskip
  {\footnotesize\color{nccblue}\centering Each frontier \emph{cluster} has its own colour, the robot targets the best utility $g=\text{gain}-\lambda\,\text{cost}$, and you \emph{see} the coloured rim eat into the unknown up to $>90\,\%$. Captures: \texttt{gazebo\_capture.mp4} \& \texttt{rviz\_capture.mp4}.}
\end{frame}

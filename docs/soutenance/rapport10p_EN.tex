...
architecture, the theoretical foundations, the team's decision journey across
...
dangerous for people - a collapsed building, a flooded basement - can a robot go in alone, understand a
space it has never seen, and report \emph{where the victims are}? This project is a self-contained answer:
it ties together SLAM, autonomous exploration, navigation and decision-making into a single system that
runs from one command and needs no human in the loop.
...
of the rooms or the victims' locations. The next sections describe the architecture, the concepts it applies, and how we got there.
...
is what makes the system easy to reason about, and it is what the next two sections unfold - first the concepts the mechanisms rest on, then the mission the policy assembles from them.
...
\section{Theoretical foundations}\label{sec:concepts}
\head{SLAM and loop closure.} The robot builds the map online with \texttt{slam\_toolbox} (pose-graph
SLAM). Loop closure corrects accumulated drift when the robot
re-observes a known place, keeping the global map consistent during repeated passes~[4,9].

\head{Frontier exploration and information gain.} A \emph{frontier} is the boundary between
...
reaches $0.85\pm0.08$ coverage (the only one above $90\,\%$) vs.\ $0.72$ for greedy. \emph{Why} the gap
widens \emph{over time} (Fig.~\ref{fig:bench}): greedy, drawn to the nearest frontier, keeps refining the
already-open area and defers distant rooms, so it plateaus ($\sim$67--70\,\%); information-gain prefers the
frontier that reveals the \emph{most unknown per metre} (a doorway opening a whole room), so each move
buys more coverage and the curve crosses $90\,\%$ - and since the cost is the Nav2 path length (not
Euclidean), walls are accounted for. A key robustness
...
\caption{Frontier exploration in RViz, the shape and colour of every element:
\textbf{coloured cubes} = frontier cells (free/unknown boundary), \textbf{one colour per cluster}
(connected component); \textbf{same-colour spheres} = cluster centroids; \textbf{large yellow sphere} =
the chosen goal frontier ($g=\mathrm{gain}-\lambda\,\mathrm{cost}$); \textbf{blue line} = the Nav2 plan;
\textbf{RGB axes} = the robot (TF). Light grey = known-free, blue-grey = unknown, black/red = walls.}
\label{fig:rviz-frontiers}
\end{figure}

\head{Path planning and control.} The global planner produces a geometric path; the local
...
\caption{Bonus ($n{=}3$) - map coverage \emph{over time}: \textbf{information-gain} (orange) reaches the
90\,\% target, \textbf{greedy} (blue) plateaus at $\sim$67--70\,\% (reasons detailed in the text).}
\end{subfigure}\hfill
\begin{subfigure}{0.55\linewidth}\centering\footnotesize
\begin{tabular}{@{}lll@{}}
\toprule
Planner & Property & Status in \texttt{bl}\\
\midrule
...
NavFn does not.
\end{subfigure}
\caption{Exploration strategy (bonus) and global-planner comparison.}
\label{fig:bench}
\end{figure}
...
\end{subfigure}
\caption{Two key concepts: pose-graph SLAM with loop closure and frontier exploration
...
\head{Concepts \emph{applied} in the project.} Table~\ref{tab:concepts} maps each topic
to where it lives in our system.

\begin{table}[htbp]\centering\footnotesize
\begin{tabular}{@{}p{0.30\linewidth}p{0.66\linewidth}@{}}
\toprule
\textbf{Concept} & \textbf{Where it is applied in \texttt{bl}}\\
\midrule
SLAM + loop closure & \texttt{slam\_toolbox} (async): online \texttt{/map} + \texttt{map}$\to$\texttt{odom} TF, loop closure on revisits\\
Frontier exploration & \texttt{frontier\_explorer\_node}: free/unknown boundary, world-keyed blacklist\\
Information gain (bonus) & info-gain frontier selection; quantitative benchmark vs.\ greedy ($n{=}3$)\\
Path planning & Nav2 global planner - NavFn (Dijkstra) kept; A* (Smac) compared\\
Local control & Nav2 DWB controller (sampling-based trajectory scoring)\\
...
\bottomrule
\end{tabular}
\caption{The key concepts, and exactly where each one is applied in the project.}
\label{tab:concepts}
\end{table}
...
deliverable artefacts. All come from the single final run (\texttt{results/examples/l18\_nominal}). A full-mission demo video is available: \url{https://www.youtube.com/watch?v=BKXprWFloFs}.
...
\begin{figure}[htbp]
\centering
\begin{subfigure}{0.33\linewidth}\includegraphics[width=\linewidth]{coverage_curve}\caption{Coverage \& path vs.\ time (90\,\% target).}\end{subfigure}\hfill
\begin{subfigure}{0.33\linewidth}\includegraphics[width=\linewidth]{trajectory_map}\caption{Real path (time-coloured), threading doorways.}\end{subfigure}\hfill
\begin{subfigure}{0.32\linewidth}\includegraphics[width=\linewidth]{mission_map}\caption{Final map: 4 victims + inspection tour.}\end{subfigure}
...
\begin{figure}[htbp]
\centering
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{mission_timeline}\caption{BT-decided timeline: Phase~1 (blue) / Phase~2 (orange) bands and the instant each victim is detected.}\end{subfigure}\hfill
\begin{subfigure}{0.30\linewidth}\includegraphics[width=\linewidth]{annotated_map_hd}\caption{Deliverable: final map marked with the victims.}\end{subfigure}\hfill
\begin{subfigure}{0.185\linewidth}\includegraphics[width=\linewidth]{rviz_live}\caption{Live RViz.}\end{subfigure}
...
We warmly thank \textbf{Zhi Yan}, Teacher-Researcher at ENSTA, for the IA712 module and guidance that
...
\bibitem{assign} IA712 Mobile Robotics - Final Project, Project~B (assignment). Site: \url{https://yzrobot.github.io/IA712/}.
...
\bibitem{cm} IA712 Mobile Robotics - reference material, \url{https://yzrobot.github.io/IA712/}.
